...
| F-24 | MDWEB-870 | Academic Add/Extend Order Summary 노트 Case 포맷 재작성 + Enterprise Order Summary 노트 신설 | Figma Description | 검토대기 | Figma `PeCid7uJcg0HenViaaiHUp` / page `Order/Checkout ✅` — Academic 4프레임 노트(`8555:1913`, `8556:1931`, `8547:2589`, `8554:1895`), Enterprise `Checkout Enterprise Team - Add/Extend`(`7905:5757`) 신설 노트 `8732:1916` + Screen 배지 `8732:1923` | Josh 직접 지시 (2026-09-07). 계산 서술을 Josh Case 포맷으로 재작성: Case 제목, 이유 문장(제목 바로 아래), Add 여부와 Extend 여부와 Seat 수 필드, 계산식, UI 표시 블록(구간 행별 취소선과 병기 금액, 칩, 캡션, Subtotal). 화면 금액 전수 대조 후 기입(2,100 취소선 1,050 / 6,000 취소선 3,000 / 7,800 취소선 3,900). Enterprise에는 4번 Order Summary 참조 노트(공통 구성, 기간 칩, Seat 할인 없음) 신설. 개발과 디자인 공유용 슬랙 초안은 채팅으로 전달, Josh가 직접 전송. 포맷 규약은 patterns.md 09-07 4차 항목. **09-08 추가**: Individual Trial과 Student Benefit applied도 Subtotal 취소선 모델로 전환(정가 280 / 8.25 취소선 + 0 USD 병기, Benefit은 VAT와 Total도 0 USD로). 화면 2곳(`7782:28345`, `7793:1681`)과 컴포넌트 변형 8, 10, 노트 5곳(변형 8과 10, 화면 노트 2, 구성 노트) 갱신. **09-08 변수 사전**: 공통 변수 4개({nextPaymentAmount}, {nextPaymentDate}, {couponRate}, {couponMonths})는 구성 노트에, Academic 전용 5개({currentSeats}, {addedSeats}, {totalSeats}, {remainingDays}, {n})는 적용 프레임 노트의 변수: 블록으로 (Josh 지시: 공통 아님). 쿠폰 캡션 변수 표기 정규화, 노트 제목 Case N 형식은 Josh 직접 개명(본문 중복 Case 줄 삭제). **09-08 2차**: ①Add/Extend 4프레임 Description 재구조화(Josh 제안): Page Context가 케이스 정의 소유, UI 항목별 노트(Extra Seats 행, Extension 행, Subtotal)와 행별 Screen 배지. ②컴포넌트 Screen 예시 숫자 전부 {} 변수화(Josh 지시), 신규 변수 {total}과 {regularPrice}와 행 표기 템플릿을 구성 노트 등재, 규칙값 0 USD는 리터럴 유지, 배너 텍스트 2줄 랩과 좌측 FILL 전환으로 잘림 해소. **09-08 3차**: 단어 "행" 전면 제거(Josh 지시, 요소명 그대로 또는 "항목"), VAT 항목 노트 4프레임 추가(할인 반영 후 Subtotal에 세율 적용 + 검산식), Page Context 스타일은 Josh 개정판(제목 Case N, 필드 서브불릿)으로 4프레임 통일. **09-08 4차**: Next Payment 배너 새 디자인(주황 테두리, 주황 14% 배경, 주황 Medium 텍스트, Josh가 컴포넌트 시트에서 확정) 화면 6곳(`8531:1707`, `1715`, `1723`, `1731`, `1739`, `1747`)에 통일 적용, 더미 데이터 유지. Manager 세션 부재로 직접 기재 | 2026-09-08 |

| F-25 | MDWEB-870 | VAT Number를 Billing Address로 편입: 화면과 Description 정리 | Figma Wireframe | 검토대기 | Figma `PeCid7uJcg0HenViaaiHUp` / page `Order/Checkout ✅` — standalone 필드 삭제 5곳(Checkout Enterprise Team `7172:3150`, Enterprise Team Add/Extend `7905:5757`, Enterprise Single `7782:28043`, COMPONENTS: BILLING INFORMATION EU 변형, 페이지 낱장 Screen `8379:4103`), Description 갱신 9곳, Case Matrix 라벨 `8166:1860` | Josh 직접 지시 (2026-09-08). 새 모델(Josh가 먼저 구축): 입력은 Add Address 모달의 VAT Number 필드(`8489:1946`), 표시는 Billing Address 카드 안 VAT Number 줄(`8489:2007`). standalone 입력은 폐지. 노출 조건: Enterprise 플랜이고 국가를 EU 회원국으로 선택한 경우에만 모달에 노출, Academics 미노출. 역과세 로직(입력 시 Reverse Charge 전환, D-1 플래그)은 새 노트로 이관. 케이스 매트릭스 노출 데이터는 유지, 라벨만 VAT Number (Billing Address 모달)로. Manager 세션 부재로 직접 기재 | 2026-09-08 |

| F-26 | MDWEB-870 | Checkout 기획 문서 3종 (PRD, 기능명세, Version Table) Figma 생성 | Figma Document | 검토대기 | Figma `PeCid7uJcg0HenViaaiHUp` / page `Order/Checkout ✅`(`237:3132`) — `Order/Checkout Version Table`(`8872:1705`), `Order/Checkout PRD 1`(`8873:1705`), `Order/Checkout PRD 2`(`8874:1705`), `Order/Checkout 기능명세`(`8875:1705`, 39행). User Flow 초안: 에디터 아티팩트 `7b03170c` 버전 checkout-flow-draft | Josh 직접 지시 (2026-09-09). `/figma-document` 첫 실행. 화면 ID `CO` 최초 발급 (화면 4종 + 모달 5종, PRD Screen Structure가 발급 원본). 근거: `docs/policy/checkout.md`(정본), `prd-draft.md` 부분 재사용(배경, 범위 표, 에러 패턴 표만), 게시본 Canvas `F0BL0SRE4TZ`. Josh 확정 4건: ①기능명세는 정책 기준 전체 커버(RESULT, BLOCKED 축 포함) ②User Flow 차트는 에디터 초안 게시까지, Josh 수정 후 PNG 삽입은 후속 ③단어 Step 금지, 화면명으로 표기(SW Account 지정 화면, 결제 화면) ④기능명세 사용자 열은 로그인 대신 Member. 신규 정책 확정 1건: 개인 계열 동일 플랜 보유 시 차단 모달(`You Already Have This Plan`, `CO-100-300-P01`). G25(추가 Seat 만료일)와 G22(Indie 대기 표시)는 원장 확인으로 해소 처리, G28과 G31은 PRD 플래그로 이관. spec.md 정정 2건: §11.5 docText 순서 버그, §9.5 사용자 컬럼 예시. **후속 완료 (2026-09-09 Josh OK)**: ①checkout.md 하단 관련 문서 블록을 새 문서 3종 node-id와 현행 페이지명으로 교체 ②Step 표기 전면 교체: 정책 문서 3개(checkout.md 13곳, plan.md 1곳, plan-card.md 1곳)와 게시본 Canvas `F0BL0SRE4TZ` 12개 섹션(KO 6, EN 6)을 화면명(SW Account 지정 화면, 결제 화면)으로 전환. Canvas 해당 섹션의 구 모델 잔재(판정 표, [다음] 버튼명)는 게시본 후속 정리 건으로 유지. **User Flow 차트 2개(진입 판정과 결제, SW Account 지정과 Purchase Type)는 에디터 초안(0.3.00)까지만: Josh 수정과 PNG 삽입 왕복은 이번 라운드 제외(09-09 Josh 결정), PRD User Flow 섹션의 삽입 자리 표시는 유지.** 플로우 도출 기준은 spec.md §9.8 신설(1.9.00). Manager 세션 부재로 직접 기재 | 2026-09-09 |
