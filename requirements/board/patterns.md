...
- **★ 2026-09-07 Seat 할인 캡션 체계 개정 (Josh 확정).** Seat 할인 안내를 Discount 행 캡션에서 **Sub Total Breakdown 행 캡션**으로 옮겼다. Discount 행의 `50% off for 10 seats or above` 캡션은 폐기. 쿠폰 캡션(`n% off for n months applied`)과 Student Benefit 캡션은 Discount 행에 그대로 남는다
    ...
    - **Student Benefit applied도 Discount 미사용 (Josh 확정, 09-07).** 컴포넌트 변형 10과 화면 `7793:1681`: Discount 0, 캡션 `Student Benefit applied` 삭제, 정가 요약 그대로(Subtotal 8.25, VAT + 0.83, Total 9.08). 오늘 무료라는 사실은 First Payment 배너(금액 9.08로 Total과 일치)와 약관이 전달. 취소선도 쓰지 않음: 개인 계열은 Breakdown 구간 행이 없다. **Trial(변형 9)은 Subtotal 0 모델 유지** — 플랜 미개시 상태라 성격이 다름 (→ **09-08 개정으로 대체**, 아래 항목 참조)
    - **★ 09-08 개정 — Trial과 Benefit도 Subtotal 취소선 (Josh 확정).** 개인 계열 정책 할인(Individual Trial, Student Benefit applied)은 Breakdown 구간 행이 없으므로 **Subtotal 행에 정가 취소선 + 0 USD 병기**로 표시한다. Trial: 정가 280 취소선(화면 `7782:28345` Subtotal `8255:1587` + 변형 8 `8254:1583`), VAT와 Total은 0 USD 그대로. Benefit applied: 정가 8.25 취소선(화면 `7793:1681` `8256:1635` + 변형 10 `8254:1630`), 과세표준이 0이라 VAT `+ 0.83`에서 `+ 0 USD`로, Total `9.08`에서 `0 USD`로 변경. 첫 청구(308 / 9.08)는 First Payment 배너가 전달. 취소선 스타일은 Academic 구간 행과 동일(회색 0.638 + STRIKETHROUGH, 단일 노드 range). 구성 노트에 "Trial과 Student Benefit은 구간 행이 없어 Subtotal 행에 정가 취소선과 0 USD를 병기" 한 줄 추가. Description 갱신: 변형 8과 10 케이스 노트, Benefit 화면 노트 `8259:1630`, Trial 화면 노트 `8259:1603`
    - **★ 09-08 변수 사전 확장 (F-24).** 변수 정의는 **적용 범위 기준으로 나눈다** (Josh 확정): 공통 변수만 구성 노트(`8025:1555`)에, Add/Extend 전용 변수는 쓰이는 프레임 노트에 둔다. 구성 노트에 추가된 공통 4개: {couponRate}, {couponMonths}(쿠폰, {discount} 뒤), {nextPaymentAmount}, {nextPaymentDate}({tax} 뒤). Academic 전용 5개({currentSeats}, {addedSeats}, {totalSeats}, {remainingDays}, {n})는 구성 노트에 넣었다가 뺐고, 각 프레임 노트의 `변수:` 블록(계산식 줄 다음, 2단 불릿)으로 이동: Add/Extend 4프레임은 {remainingDays} 공통 + 해당 시 {totalSeats}(Case 3, 4)와 {n}(미달 캡션 있는 Case 1, 3), New1 Seat 계산 박스 노트는 {n}. 쿠폰 캡션 문구도 맨 n 대신 변수 표기로 정규화: `{couponRate}% off for {couponMonths} months applied`. 케이스 노트의 표시 문자열 줄은 실제 예시 값을 유지. 노트 제목은 Josh가 `Case N: {제목}` 형식으로 직접 개명 — 본문의 Case 줄은 제목 반복이라 삭제
    - **★ 09-08 케이스 프레임 재구조화 (Josh 제안).** Add/Extend 케이스 프레임 Description 구조: **Page Context가 케이스 정의를 소유**(진입 조건 2줄, COMPONENTS 참조 줄, Case 제목, 이유 문장, Add 여부와 Seat 수 필드) + **UI 항목별 번호 노트**(① Extra Seats 행 ② Extension 행, 해당 시 ③ Subtotal). 각 행 노트는 계산식, 변수 인라인 정의, 취소선과 병기 금액, 칩, 캡션 순. Screen 배지는 Order Summary 카드 좌측 거터 한 열에 행별 정렬. 주의: 행 라벨 검색 시 왼쪽 Seat 컨트롤에도 Extra Seats 라벨이 있어 **가장 오른쪽 매치**를 잡아야 한다
    - **★ 09-08 컴포넌트 시트 변수화 (Josh 지시).** 컴포넌트 Screen(`7992:1577`)의 예시 숫자를 전부 {} 변수 표기로 교체: {subtotal}, {tax}, {total}, -{discount}, {seatCount}, {unitPrice}, {taxRate}, 캡션 {n}과 {couponRate}% off for {couponMonths} months applied, 배너 템플릿 문자열({nextPaymentAmount}, {nextPaymentDate}), Add/Extend 행 {addedSeats}, {proratedPrice}, {addAmount}, {totalSeats}, {years}, {extendAmount}, 취소선 쌍은 {regularPrice} + 할인 후 금액. **규칙값 0 USD는 리터럴 유지.** 신규 변수({total}, {regularPrice})와 행 표기 템플릿 3줄은 구성 노트에 등재. 레이아웃: 좌측 프레임 FILL 전환 8행, 배너 텍스트 layoutSizingHorizontal FILL(2줄 랩, 인스턴스 자식에도 적용 가능 확인), 포뮬러 행 layoutWrap WRAP. 변수명은 금액 컬럼 폭에 맞게 짧게({extraSeatsAmount}가 아니라 {addAmount})
    - **★ 09-08 용어 — 단어 "행" 금지 (Josh 지시).** Description과 산출물 전반에서 표 줄을 "행"으로 부르지 않는다. 요소명을 그대로 쓰고(Extra Seats, Extension, Subtotal, Discount), 집합 지칭이 필요하면 "항목"("각 항목은 이름과 산식을 두 줄로"), 캡션 위치는 "Seats 아래"처럼 요소명 + 아래. 페이지 전체 22개 텍스트와 노트 제목 6개에서 제거 완료, 유일하게 남은 "행"은 "결제 실행 CTA"의 실행
    - **★ 09-08 VAT 노트 추가 (Josh 지시).** Add/Extend 케이스 프레임의 항목별 노트에 VAT 포함: "할인 반영 후 Subtotal에 청구지 세율 10% 적용" + 케이스 검산식. 세금이 할인 후 금액 기준이라는 게 핵심 규칙. 4프레임에 노트와 배지 추가
    - **★ 09-08 Page Context 최종 스타일 (Josh 직접 개정).** 제목이 "Case N: {제목}", 본문은 진입 조건 2줄 + 이유 문장 + 필드 서브불릿(Add 여부, Seat 수, 연장). COMPONENTS 참조 줄과 본문 Case 줄은 제거(제목이 말함). Josh가 Case 2~4를 직접 다듬었고 Case 1은 같은 패턴으로 맞춤
    - **★ 09-08 배너 디자인 확정 (Josh 직접 개정).** Next Payment 배너: 주황(0.886, 0.349, 0.153) 테두리 1px + 같은 주황 14% 배경 + 모서리 8 + 텍스트 Poppins Medium 주황. 구 디자인(하늘색 테두리, 노란 배경)은 폐기. 컴포넌트 시트 6개는 {} 템플릿, 화면 6개는 더미 데이터. 12개 전수 통일 확인
    - **★ 09-08 VAT Number의 Billing Address 편입 (Josh 확정).** standalone VAT number 입력은 폐지. 입력은 Add Address 모달의 VAT Number 필드(Enterprise 플랜이고 국가를 EU 회원국으로 선택한 경우에만 노출, Academics 미노출), 표시는 Billing Address 카드 안 "VAT Number: DE123456789" 줄. 더미 주소가 미국인 화면 카드에는 표시 없음이 정합, EU 케이스는 COMPONENTS: BILLING INFORMATION EU 변형이 정본. 역과세 로직(선택 입력, 미입력 시 표준 세율, 입력 시 Reverse Charge 전환, 국가 변경 시 초기화, 무효 처리 D-1)은 유지되고 위치 서술만 바뀜
    - **★ 09-07 4차 — 계산 케이스는 Case 포맷 (Josh 확정, F-24).** Add/Extend류 Order Summary 노트의 계산 서술은 변수 기호 나열이 아니라 **화면 예시 값 + Case 포맷**으로 쓴다. 구조: `Case N: {제목}` 다음 줄에 이유 문장(레이블 없이), 이어서 `Add 여부 / Extend 여부 / 현재 Seats / 추가 Seats / (총 Seats) / (연장) / (할인) / 계산식` 필드, 마지막에 `UI 표시:` 블록(2단 불릿: 구간 행별 취소선과 병기 금액, 칩, 캡션, Subtotal). 첫 줄 컴포넌트 참조는 유지. 적용: Academic Add/Extend 4프레임(`8555:1913`, `8556:1931`, `8547:2589`, `8554:1895`). Enterprise Add/Extend에는 ④ Order Summary 참조 노트 신설(`8732:1916`, Screen 배지 `8732:1923`)
