...
### S8 — CLO Admin 등록 프로세스
*CLO는 MD와 달리 한국어를 우선 등록하는 프로세스가 필요. 현재 UI만 있고 실제 등록 불가.*

**MD와의 차이점**
- MD: 언어 순서 무관하게 등록 가능
- CLO: 한국어 먼저 등록 → 이후 다국어 확장 구조여야 함

**논의 필요 항목**
- 한국어 필드 미입력 시 저장 차단 여부 (하드 블로킹 vs 경고)
- MD Admin과 동일한 UI를 재사용할 수 있는지, CLO 전용 프로세스가 필요한지
- 등록 완료 기준 정의 (한국어만 있으면 완료인지, 특정 필드 필수 여부)

- [ ] 논의 완료 후 프로세스 확정
- [ ] CLO Admin 한국어 우선 등록 구현 (S9 선행 조건)
